...
const updateQuery = (obj, position, condition, by) => {
  ...
  const splitTitle = splitPosition[0].split('=');
  const splitArticle = splitPosition[1].split('=');
  const set = `${splitTitle[0]}=$1, ${splitArticle[0]}=$2`;


  const splitCondition = condition.split('=');
  const where = `${splitCondition[0]}=$3`;

  const valueOne = splitTitle[1];
  const valueTwo = splitArticle[1];
